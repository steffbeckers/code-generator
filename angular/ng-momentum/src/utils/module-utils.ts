...
export function addRouteToAppRoutingModule(host: Tree, options: HasImportOptions & BaseOptions): Tree {
  ...
  const loadChildren =
    '../app/' + normalize(childrenPath + '/' + strings.dasherize(options.name) + '.module#' + strings.classify(options.name) + 'Module');
  // read target file
...
export function addImportToNgModule(host: Tree, options: HasImportOptions & BaseOptions, classifiedName: string): Tree {
  ...
  const insertPath = `/${options.path}/` + (options.flat ? '' : strings.dasherize(options.name) + '/') + strings.dasherize(options.name) + '.module';
  const relativePath = buildRelativePath(modulePath, insertPath);
  const declarationChanges = addImportToModule(source, modulePath, classifiedName, relativePath);
  ...
  if (options.export) {
    ...
    const exportChanges = addExportToModule(source, modulePath, strings.classify(`${options.name}Module`), relativePath);
