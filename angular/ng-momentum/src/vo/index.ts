import { Rule, SchematicContext, Tree, apply, chain, template, url, move, noop, filter, mergeWith, MergeStrategy } from '@angular-devkit/schematics';
...
export function vo(options: VoOptions): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    const movePath = options.flat
      ...
      : join(options.path, constants.voFolder, strings.dasherize(strings.singularize(options.name)));
    // get template source
