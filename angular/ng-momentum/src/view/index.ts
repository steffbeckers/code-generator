...
export const VIEW_OPTIONS = [VIEW_OPTION.Blank, VIEW_OPTION.List, VIEW_OPTION.DETAILS, VIEW_OPTION.FORM, VIEW_OPTION.TABLE];
...
export function view(options: ViewOptions): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    options.servicePath = options.servicePath
      ? options.servicePath
      : join(options.path, constants.servicesFolder, strings.dasherize(strings.pluralize(options.service)));
    ...
    const rule = chain([
      options.skipVo
        ? noop()
        : schematic(constants.voSchematic, {
            name: options.vo,
            spec: options.spec,
            obj: options.obj,
          }),
      ...
      options.skipService
        ? noop()
        : schematic(constants.serviceSchematic, {
